...
impl Mapping {
    pub fn object_property_triples<F,T>(
        &self, func: F, out:&mut Vec<T>
    )
    where F: Fn(&str, &str, &str) -> T{
    ...
    pub fn data_property_triples<F,T>(
        &self, func: F, out:&mut Vec<T>
    )
    where F: Fn(&str, &str, &str, Option<&str>, &str) -> T { //subject, verb, lexical_form, language_tag, datatype
    ...
    pub fn export_oxrdf_triples(&self) -> Vec<Triple> {
        ...
        fn data_triple_func(s:&str, v:&str, lex:&str, lang_opt:Option<&str>, dt:&str) -> Triple {
            let subject = subject_from_str(s);
            let verb = NamedNode::new_unchecked(v);
