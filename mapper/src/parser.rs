...
fn parameter(p: &str) -> IResult<&str, Parameter> {
    ...
    if let Ok(_) = peek(double_question_mark_case())(p).finish() {
        let (p, (_, opt_mode, _, stottr_variable, _, default_value, _)) =
            double_question_mark_case()(p)?;
        ...
        let (p, (_, opt_mode, _, ptype, _, variable, _, default_value, _)) = tuple((
            multispace0,
